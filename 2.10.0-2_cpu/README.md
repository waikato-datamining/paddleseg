...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    ...
    -it public.aml-repo.cms.waikato.ac.nz:443/paddle/paddleseg:2.10.0-2_cpu
  ```
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/paddleseg:2.10.0-2_cpu
  ```

### Build local image

* Build the image from Docker file (from within /path_to/paddleseg/2.10.0-2_cpu)
...
```bash
docker build -t paddleseg:2.10.0-2_cpu .
```
...
* Tag
  ...
  ```bash
  docker tag \
    paddleseg:2.10.0-2_cpu \
    public-push.aml-repo.cms.waikato.ac.nz:443/paddle/paddleseg:2.10.0-2_cpu
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/paddle/paddleseg:2.10.0-2_cpu
  ```
...
* Tag
  ...
  docker tag \
    paddleseg:2.10.0-2_cpu \
    waikatodatamining/paddleseg:2.10.0-2_cpu
  ```
  
* Push

  ```bash
  docker push waikatodatamining/paddleseg:2.10.0-2_cpu
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
