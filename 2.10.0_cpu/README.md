...
Uses PyTorch 1.11.0 and PaddleSeg 2.10.0.
...
* Pull and run image (adjust volume mappings `-v`):
  ...
  docker run --shm-size 8G --net=host \
    -v /local/dir:/container/dir \
    -it waikatodatamining/paddleseg:2.10.0_cpu
  ```

### Build local image

* Build the image from Docker file (from within /path_to/paddleseg/2.10.0_cpu)
...
```bash
docker build -t paddleseg:2.10.0_cpu .
```
...
* Tag
  ...
  ```bash
  docker tag \
    paddleseg:2.10.0_cpu \
    public-push.aml-repo.cms.waikato.ac.nz:443/paddle/paddleseg:2.10.0_cpu
  ```
  
* Push

  ```bash
  docker push public-push.aml-repo.cms.waikato.ac.nz:443/paddle/paddleseg:2.10.0_cpu
  ```
...
* Push
  ...
  ```bash
  docker push waikatodatamining/paddleseg:2.10.0_cpu
  ```
  If error "no basic auth credentials" occurs, then run (enter username/password when prompted):
